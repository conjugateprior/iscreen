...
makeDiscrete<-function(C){
  ...
  res <-  foreach(xx=1:ncol(C),.packages='Matrix',.export='xdiscrete',.combine='cbind') %dopar% {
    xdiscrete(C[,xx])
  }
...
#calculate I for all marginals of variables
calcI<-function(C,y){
  res <- foreach(xx=1:ncol(C),.packages='Matrix',.export='f.list.I',.combine=c) %dopar% {
...
#calculate I for all 2-way interactions
calcI2<-function(C,y,ind){#ind is combn(k,2) matrix
  res <- foreach(xx=1:ncol(ind),.packages='Matrix',.export='f.list.I',.combine=c) %dopar% { #rbind
...
get.predictors.int <- function(x,y,inds,nclus=200){
  ...
  cl <- makeCluster(cc,type=ifelse(.Platform$OS.type=="unix","FORK","PSOCK"))
  registerDoParallel(cl)
  res <- foreach(j=1:nclus,.packages='Matrix',.export='getX2',.combine=cbind) %dopar%{
    getX2(x[,inds[1:2,j]])
  }
  stopCluster(cl)
  return(res)
